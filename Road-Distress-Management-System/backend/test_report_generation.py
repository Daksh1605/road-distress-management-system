"""
Automated Integration Verification: test_report_generation.py
Verifies the PDF Report Generation System end-to-end: uploading a video, running detection, generating a PDF report, fetching reports list, downloading the report, previewing the report, and cleaning up database entries and files.
"""
...
def create_temp_video(filename="temp_test_report_run.mp4"):
    """
...
def run_tests() -> dict:
    ...
    logger.info("=== STARTING PDF REPORT GENERATION INTEGRATION TEST ===")

    temp_video_name = "temp_test_report_run.mp4"
    try:
    ...
    try:
        headers, body = encode_multipart_formdata(
            fields={},
            files=[("file", "temp_test_report_run.mp4", video_bytes, "video/mp4")]
        )
    ...
    if not is_completed:
        record_test("PDF Report Generation", False, "Skipping: AI processing failed or timed out.")
    else:
        # 4. Generate PDF Report
        try:
            status, resp = http_request(f"/reports/generate/{video_id}", method="POST")
            passed = (status == 201 and "filepath" in resp and resp.get("filepath") is not None)
            report_id = resp.get("id")
            record_test("POST /reports/generate/{video_id} - Generate PDF", passed, f"Status {status}, Response {resp}")
        except Exception as e:
            record_test("POST /reports/generate/{video_id} - Generate PDF", False, str(e))

    if report_id:
        # 5. Verify PDF download
        try:
            status, raw_bytes = http_request(f"/reports/download/{report_id}", return_raw=True)
            # Verify file signature for PDF (%PDF-...)
            passed = (status == 200 and raw_bytes.startswith(b"%PDF"))
            record_test("GET /reports/download/{report_id} - Verify PDF Content Signature", passed, f"Status {status}, Bytes length {len(raw_bytes)}")
        except Exception as e:
            record_test("GET /reports/download/{report_id} - Verify PDF Content Signature", False, str(e))

        # 6. Verify PDF preview
        try:
            status, raw_bytes = http_request(f"/reports/preview/{report_id}", return_raw=True)
            passed = (status == 200 and raw_bytes.startswith(b"%PDF"))
            record_test("GET /reports/preview/{report_id} - Verify Inline Preview", passed, f"Status {status}")
        except Exception as e:
            record_test("GET /reports/preview/{report_id} - Verify Inline Preview", False, str(e))

        # 7. Get single report log
        try:
            status, resp = http_request(f"/reports/{report_id}")
            passed = (status == 200 and resp.get("id") == report_id)
            record_test("GET /reports/{id} - Get Report Log", passed, f"Status {status}, Response {resp}")
        except Exception as e:
            record_test("GET /reports/{id} - Get Report Log", False, str(e))

        # 8. List reports
        try:
            status, resp = http_request("/reports/")
            passed = (status == 200 and isinstance(resp, list) and any(r.get("id") == report_id for r in resp))
            record_test("GET /reports/ - List All Reports", passed, f"Status {status}, Count {len(resp)}")
        except Exception as e:
            record_test("GET /reports/ - List All Reports", False, str(e))
    ...
    # 10. Clean up video logs and files
    if video_id:
        try:
    ...
    # 11. Clean up temporary video source file
    if os.path.exists(temp_video_name):
        try:
            os.remove(temp_video_name)
    ...
    logger.info("=== PDF REPORT GENERATION INTEGRATION TEST COMPLETED ===")
    return results
...
if __name__ == "__main__":
    ...
    print(f"\nPDF Report Generation Test Score: {score}/100")
